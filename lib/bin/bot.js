...
async function searchbooru(channelID,typee,stuff){
	...
	console.log(stuff[1])
	  ...
	  if (typee=="pony"){
		 console.log(stuff)
		 booru.search("derpi", stuff, {limit: 1, random: true}) //currently only supports single tags
		.then(images => {
		//Log the direct link to each image
		for (let image of images) {
		console.log(image.common.file_url)
		if(illegalcontent(image.common.file_url)==false){
		console.log(image.common.file_url)
		bot.sendMessage({
		to:channelID,
		message:image.common.file_url})}else{searchbooru(channelID,typee,stuff)}
	}
		})
			.catch(err => {
	  ...
	  if (typee=="e621"){
		  console.log(stuff)
		 booru.search("e6", stuff, {limit: 1, random: true}) //currently only supports single tags
		.then(images => {
		//Log the direct link to each image
		for (let image of images) {
		//console.log(image.common)
		console.log(image.common._data.tags)
		if(illegalcontent(image.common._data.tags)==false){
		console.log(image.postView)
		bot.sendMessage({
		to:channelID,
		message:image.common.file_url})}else{searchbooru(channelID,typee,stuff)}
	}
